...
@IBDesignable open class KaedeTextField: TextFieldEffects {
    ...
    override open func animateViewsForTextEntry() {
        ...
        } else if self.alignType == .flexibleRightAligned {
            UIView.animate(withDuration: 0.35, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 2.0, options: .beginFromCurrentState, animations: ({
                self.placeholderLabel.frame.origin = CGPoint(x: self.frame.size.width - self.placeholderLabel.intrinsicContentSize.width - self.placeholderInsets.x, y: self.placeholderInsets.y)
            }), completion: nil)

            UIView.animate(withDuration: 0.45, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 1.5, options: .beginFromCurrentState, animations: ({
                self.foregroundView.frame.origin = CGPoint(x: self.frame.size.width - self.placeholderLabel.intrinsicContentSize.width - 2 * self.placeholderInsets.x, y: 0)
            }), completion: { _ in
                self.animationCompletionHandler?(.textEntry)
            })
        }
    ...
    override open func editingRect(forBounds bounds: CGRect) -> CGRect {
        if alignType == .flexibleRightAligned {
            let frame = CGRect(origin: bounds.origin, size: CGSize(width: bounds.size.width - placeholderLabel.intrinsicContentSize.width - textFieldInsets.x, height: bounds.size.height))
            if frame.height == 100.0 {
                return frame.insetBy(dx: 0 , dy: 0)
            } else {
                return frame.insetBy(dx: 10 , dy: 0)
            }
        }
